...
def visualize_TeX(TeX, fname, max_vals, kind="pred"):
    ...
    TeX_ = (TeX_ - np.min(TeX_)) / (np.max(TeX_) - np.min(TeX_) * 255.0).astype(
        np.uint8
    )
    plt.imshow(TeX_)
    plt.title("TeX " + kind)
...
def visualize_v(data, fname, kind="pred"):
    ...
    if kind != "KL-div":
        ...
        im = ax[0].imshow(v1, vmin=0.0, vmax=vmax, cmap="turbo")
        ax[0].axis("off")
        ax[0].set_title("v1")
        plt.colorbar(im, ax=ax[0])

        im = ax[1].imshow(v2, vmin=0.0, vmax=vmax, cmap="turbo")
        ax[1].axis("off")
        ax[1].set_title("v2")
        ...
        fig, ax = plt.subplots()
        im = ax.imshow(data, vmin=0.0, vmax=vmax, cmap="turbo")
        ax.axis("off")
...
for j in range(44):
    ...
    pred_file = torch.load(
        os.path.join(DATA_DIR, f"val_pred_{j}.pt"), map_location="cpu"
    )
    S_pred_file = torch.load(
        os.path.join(DATA_DIR, f"val_S_pred_{j}.pt"), map_location="cpu"
    )
    S_true_file = torch.load(
        os.path.join(DATA_DIR, f"val_S_true_{j}.pt"), map_location="cpu"
    )
    ...
    for i in trange(n):
        ...
        if nclass == max_n_class:
            ...
            e_ce_error = F.cross_entropy(
                e_pred.unsqueeze(0), e.unsqueeze(0), reduction="none"
            ).squeeze()
            e = e.numpy()
        ...
        if T_pred is not None:
            ...
            visualize_T(
                T_pred, fname=f"Tmap_pred_{i+j*n}.png", error=False, kind="pred"
            )
        ...
        visualize_TeX(
            TeX_pred,
            fname=f"TeX_pred_{i+j*n}.png",
            max_vals=[T_max, S_max],
            kind="pred",
        )
        visualize_TeX(
            TeX_true, fname=f"TeX_GT_{i+j*n}.png", max_vals=[T_max, S_max], kind="gt"
        )
